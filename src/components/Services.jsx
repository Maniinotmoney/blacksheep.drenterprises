...
const Card = ({ i, title, description, image, features }) => {
    // Helper to get image URL whether it's from Sanity or local import
    const getImageUrl = (img) => {
        if (!img) return '';
        if (typeof img === 'string') return img; // Local import path
        return urlFor(img).width(600).url(); // Sanity object
    };
